...
namespace TVRename.AppLogic
{
    ...
    {
        public List<ProcessedSeries> ShowItems;
        ...
        private List<FinderBase> Finders;
        ...
        public List<String> getGenres()
        {
            ...
            foreach (ProcessedSeries si in ShowItems)
            {
        ...
        public TvRenameManager(FileInfo settingsFile, CommandLineArgs args)
        {
            ...
            ShowItems = new List<ProcessedSeries>();
            ...
            this.Finders = new List<FinderBase>();
            ...
            this.LoadOK = ((settingsFile == null) || this.LoadXMLSettings(settingsFile)) && TheTvDbClient.Instance.LoadOK;
        ...
        {
            this.mStats.NumberOfShows = ShowItems.Count;
            this.mStats.NumberOfSeasons = 0;
            this.mStats.NumberOfEpisodesExpected = 0;

            this.LockShowItems();
            foreach (ProcessedSeries si in ShowItems)
            {
                foreach (KeyValuePair<int, List<ProcessedEpisode>> k in si.SeasonEpisodes)
                    this.mStats.NumberOfEpisodesExpected += k.Value.Count;
                this.mStats.NumberOfSeasons += si.SeasonEpisodes.Count;
            }
        ...
        public List<ProcessedSeries> GetShowItems(bool lockThem)
        {
            if (lockThem)
                this.LockShowItems();

            ShowItems.Sort(new Comparison<ProcessedSeries>(ProcessedSeries.CompareShowItemNames));
            return ShowItems;
        }
        ...
        {
            return FindSeasEp(theFile, out seasF, out epF, out maxEp, sI, out FilenameProcessorRegEx rex);
        }
        ...
        public bool CheckFolderForShows(DirectoryInfo di2, bool andGuess, out DirectoryInfo[] subDirs)
        {
            ...
            try
            {
                ...
                {
                    if (ApplicationSettings.Instance.BulkAddCompareNoVideoFolders && !HasFilmFiles(di2)) return false;

                    if (ApplicationSettings.Instance.BulkAddIgnoreRecycleBin && di2.FullName.Contains("$RECYCLE.BIN", StringComparison.OrdinalIgnoreCase))
                        return true;
        ...
        {
            return directory.GetFiles("*", System.IO.SearchOption.TopDirectoryOnly).Any(file => ApplicationSettings.Instance.UsefulExtension(file.Extension, false));
        }
        ...
        public void AddAllToMyShows()
        {
            ...
            foreach (FolderMonitorEntry ai in this.AddItems)
            {
                ...
                ProcessedSeries found = this.ShowItemForCode(ai.TVDBCode);
                if (found == null)
                {
        ...
        public void GuessShowItem(FolderMonitorEntry ai)
        {
            ...
            TheTvDbSeries ser = TheTvDbClient.Instance.FindSeriesForName(showName);
            if (ser != null)
        ...
        public bool RenameFilesToMatchTorrent(string torrent, string folder, TreeView tvTree, ProgressUpdatedDelegate prog, bool copyNotMove, string copyDest, CommandLineArgs args)
        {
        ...
        public void GetThread(Object codeIn)
        {
            ...
            try
            {
                ...
                bool bannersToo = ApplicationSettings.Instance.NeedToDownloadBannerFile();

                threadslogger.Trace("  Downloading " + code);
                bool r = TheTvDbClient.Instance.EnsureUpdated(code, bannersToo);
        ...
        public void Downloader()
        {
            ...
            try
            {
                ...
                int numWorkers = ApplicationSettings.Instance.ParallelDownloads;
        ...
        public void TidyTVDB()
        {
            ...
            foreach (KeyValuePair<int, TheTvDbSeries> kvp in TheTvDbClient.Instance.GetSeriesDict())
            {
                bool found = false;
                foreach (ProcessedSeries si in ShowItems)
                {
        ...
        public List<FileInfo> FindEpOnDisk(DirectoryFileCache dfc, ProcessedEpisode pe, bool checkDirectoryExist = true)
        {
            return this.FindEpOnDisk(dfc, pe.SI, pe, checkDirectoryExist);
        }

        public List<FileInfo> FindEpOnDisk(DirectoryFileCache dfc, ProcessedSeries si, TheTvDbEpisode epi, bool checkDirectoryExist = true)
        {
            if (dfc == null)
        ...
        public bool HasAnyAirdates(ProcessedSeries si, int snum)
        {
            TheTvDbSeries ser = TheTvDbClient.Instance.GetSeries(si.TVDBCode);

            if (ser == null) return false;

            Dictionary<int, TheTvDbSeason> seasonsToUse = si.DVDOrder ? ser.DVDSeasons : ser.AiredSeasons;

            if (!seasonsToUse.ContainsKey(snum)) return false;

            foreach (TheTvDbEpisode e in seasonsToUse[snum].Episodes)
            {
        ...
        public bool GenerateEpisodeDict(ProcessedSeries si)
        {
            ...
            bool r = true;
            Dictionary<int, TheTvDbSeason> seasonsToUse = si.DVDOrder ?
                ser.DVDSeasons
                : ser.AiredSeasons;
            foreach (KeyValuePair<int, TheTvDbSeason> kvp in seasonsToUse)
            {
        ...
        public static List<ProcessedEpisode> GenerateEpisodes(ProcessedSeries si, TheTvDbSeries ser, int snum, bool applyRules)
        {
            ...
            TheTvDbSeason seas = seasonsToUse[snum];
            ...
            {
                // merge specials in
                foreach (TheTvDbEpisode ep in seasonsToUse[0].Episodes)
                {
            ...
            if (applyRules)
            {
                List<SeasonRule> rules = si.RulesForSeason(snum);
                if (rules != null)
        ...
        public static void ApplyRules(List<ProcessedEpisode> eis, List<SeasonRule> rules, ProcessedSeries si)
        {
            foreach (SeasonRule sr in rules)
            {
                int nn1 = sr.First;
                int nn2 = sr.Second;
                    ...
                    case RuleAction.Collapse:
                        {
                            if ((n1 != -1) && (n2 != -1) && (n1 < ec) && (n2 < ec) && (n1 < n2))
                            {
                                ...
                                List<TheTvDbEpisode> alleps = new List<TheTvDbEpisode>();
                                alleps.Add(eis[n1]);
        ...
        public bool LoadXMLSettings(FileInfo from)
        {
                    ...
                    while (!reader.EOF)
                    {
                        ...
                        else if (reader.Name == "MyShows")
                        {
                            ...
                            while (!r2.EOF)
                            {
                                ...
                                if (r2.Name == "ShowItem")
                                {
                                    ProcessedSeries si = new ProcessedSeries(r2.ReadSubtree());

                                    if (si.UseCustomShowName) // see if custom show name is actually the real show name
                                    {
                                        TheTvDbSeries ser = si.TheSeries();
            ...
            ApplicationSettings.Instance.LibraryFoldersNames = this.MonitorFolders;
            ApplicationSettings.Instance.IgnoreFoldersNames = this.IgnoreFolders;
            ApplicationSettings.Instance.DownloadFoldersNames = this.SearchFolders;
        ...
        public List<ProcessedEpisode> NextNShows(int nShows, int nDaysPast, int nDaysFuture)
        {
            ...
            for (int i = 0; i < nShows; i++)
            {
                ...
                foreach (ProcessedSeries si in this.GetShowItems(false))
                {
        ...
        public ActionItemQueue[] ActionProcessorMakeQueues(ItemList theList)
        {
            // Take a single list
            ...
            ActionItemQueue[] queues = new ActionItemQueue[4];
            queues[0] = new ActionItemQueue("Move/Copy", 1); // cross-filesystem moves (slow ones)
            queues[1] = new ActionItemQueue("Move/Delete", 1); // local rename/moves
            queues[2] = new ActionItemQueue("Write Metadata", 4); // writing KODI NFO files, etc.
            queues[3] = new ActionItemQueue("Download", ApplicationSettings.Instance.ParallelDownloads); // downloading torrents, banners, thumbnails

            foreach (ItemBase sli in theList)
            {
                if (!(sli is ActionItemBase action))
                    continue; // skip non-actions

                if (action is WriteMetaDataActionItem) // base interface that all metadata actions are derived from
                    queues[2].ActionItems.Add(action);
                else if ((action is DownloadImageActionItem) || (action is ActionRSS))
                    queues[3].ActionItems.Add(action);
                else if (action is CopyMoveRenameActionItem)
                    queues[(action as CopyMoveRenameActionItem).IsQuickOperation() ? 1 : 0].ActionItems.Add(action);
                else if (action is DateTouchActionItem) // add these after the slow copy operations
                    queues[0].ActionItems.Add(action);
                else if ((action is DeleteFileActionItem) || (action is DeleteDirectoryActionItem))
