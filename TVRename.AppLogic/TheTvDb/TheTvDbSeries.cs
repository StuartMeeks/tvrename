...
namespace TVRename.AppLogic.TheTvDb
{
    public class TheTvDbSeries
    {
        ...
        public Dictionary<int, TheTvDbSeason> AiredSeasons;
        public Dictionary<int, TheTvDbSeason> DVDSeasons;

        //All Banners
        public Dictionary<int, TheTvDbBanner> AllBanners; // All Banners linked by bannerId.

        //Collections of Posters and Banners per season
        private Dictionary<int, TheTvDbBanner> SeasonBanners; // e.g. Dictionary of the best posters per series.
        private Dictionary<int, TheTvDbBanner> SeasonLangBanners; // e.g. Dictionary of the best posters per series in the correct language.
        private Dictionary<int, TheTvDbBanner> SeasonWideBanners; // e.g. Dictionary of the best wide banners per series.
        private Dictionary<int, TheTvDbBanner> SeasonLangWideBanners; // e.g. Dictionary of the best wide banners per series in the correct language.
        ...
        public DateTime? LastAiredDate()
        {
            ...
            foreach (TheTvDbSeason s in this.AiredSeasons.Values) //We can use AiredSeasons as it does not matter which order we do this in Aired or DVD
            {
        ...
        public TheTvDbSeries(string name, int id)
        {
            SetToDefauts();
            this.Name = name;
            this.TVDBCode = id;
        ...
        public TheTvDbSeries(XmlReader r)
        {
            SetToDefauts();
            LoadXml(r);
        }

        public TheTvDbSeries(JObject json, int langId)
        {
            SetToDefauts();
        ...
        public TheTvDbSeries(JObject json, JObject jsonInDefaultLang, int langId)
        {
            SetToDefauts();
            this.LanguageId = langId;
        ...
        public void SetToDefauts()
        {
            ...
            this.AiredSeasons = new Dictionary<int, TheTvDbSeason>();
            this.DVDSeasons = new Dictionary<int, TheTvDbSeason>();
        ...
        public void resetBanners()
        {
            ...
            this.AllBanners = new Dictionary<int, TheTvDbBanner>();
            this.SeasonBanners = new Dictionary<int, TheTvDbBanner>();
            this.SeasonLangBanners = new Dictionary<int, TheTvDbBanner>();
            this.SeasonWideBanners = new Dictionary<int, TheTvDbBanner>();
            this.SeasonLangWideBanners = new Dictionary<int, TheTvDbBanner>();
        ...
        public void Merge(TheTvDbSeries o, int preferredLanguageId)
        {
        ...
        public void LoadJson(JObject r)
        {
            ...
            {
                if (seriesItems.Name == "aliases") this.Items[seriesItems.Name] = JsonHelper.Flatten(seriesItems.Value, "|");
                else if (seriesItems.Name == "genre") this.Items[seriesItems.Name] = JsonHelper.Flatten(seriesItems.Value, "|");
                else try
                    {
        ...
        public string GetImage(ApplicationSettings.FolderJpgIsType type)
        {
            switch (type)
            {
                case ApplicationSettings.FolderJpgIsType.Banner:
                    return GetSeriesWideBannerPath();
                case ApplicationSettings.FolderJpgIsType.FanArt:
                    return GetSeriesFanartPath();
                case ApplicationSettings.FolderJpgIsType.SeasonPoster:
        ...
        public TheTvDbSeason GetOrAddAiredSeason(int num, int seasonID)
        {
            if (this.AiredSeasons.ContainsKey(num))
                return this.AiredSeasons[num];

            TheTvDbSeason s = new TheTvDbSeason(this, num, seasonID);
        ...
        public TheTvDbSeason GetOrAddDVDSeason(int num, int seasonID)
        {
        ...
        public void AddOrUpdateBanner(TheTvDbBanner banner)
        {
        ...
        private int GetBestBannerId(TheTvDbBanner selectedBanner, int bestBannerId)
        {
        ...
        public void AddOrUpdateSeasonPoster(TheTvDbBanner banner)
        {
            AddUpdateIntoCollections(banner, this.SeasonBanners, this.SeasonLangBanners);
        }

        public void AddOrUpdateWideSeason(TheTvDbBanner banner)
        {
            AddUpdateIntoCollections(banner, this.SeasonWideBanners, this.SeasonLangWideBanners);
        }

        private void AddUpdateIntoCollections(TheTvDbBanner banner, Dictionary<int, TheTvDbBanner> coll, Dictionary<int, TheTvDbBanner> langColl)
        {
        ...
        private void AddUpdateIntoCollection(TheTvDbBanner banner, Dictionary<int, TheTvDbBanner> coll)
        {
        ...
        internal TheTvDbEpisode getEpisode(int seasF, int epF, bool dvdOrder)
        {
            if (dvdOrder)
            {
                foreach (TheTvDbSeason s in this.DVDSeasons.Values)
                {
                    if (s.SeasonNumber == seasF)
                    {
                        foreach (TheTvDbEpisode pe in s.Episodes)
                        {
            ...
            {
                foreach (TheTvDbSeason s in this.AiredSeasons.Values)
                {
                    if (s.SeasonNumber == seasF)
                    {
                        foreach (TheTvDbEpisode pe in s.Episodes)
